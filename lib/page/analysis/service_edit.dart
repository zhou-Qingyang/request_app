...
class _DataManagementPageState extends State<DataManagementPage> {
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              ...
              setState(() {
                final index = _serviceData.indexWhere(
                  (item) => item.id == serviceData.id,
                );
                if (index != -1) {
                  _serviceData[index] = serviceData.copyWith(
                    title: titleCtrl.text,
                    data: data,
                    lateDayData: lateDayData,
                  );
                }
              });
            ...
            child: Text(
              '保存',
              style: TextStyle(fontSize: 14.sp, color: const Color(0xFF3f6ff4)),
            ),
          ),
        ],
      ),
  ...
  Widget _buildQuoteOrderLimitCard() {
    return Container(
      ...
      child: ListTile(
        ...
        trailing: IconButton(
          icon: Icon(Icons.edit,size: 20, color: const Color(0xFF3f6ff4)),
          onPressed: _openQuoteOrderLimitDialog,
        ),
      ),
    );
  }
  ...
  // 报价订单限制编辑对话框
  void _openQuoteOrderLimitDialog() {
    final limitCtrl = TextEditingController(text: _quoteOrderLimit.toString());
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        backgroundColor: Colors.white,
